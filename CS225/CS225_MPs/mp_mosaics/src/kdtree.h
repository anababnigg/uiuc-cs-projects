...
class KDTree
{
  ...
  private:
    ...
    /**
     * Helper for KDTree Copy Constructor.
     * 
     * @param node the original node used for copy constructing
    */
    void copyKD(KDTreeNode* left, KDTreeNode* right);

    /**
     * Helper for KDTree Copy Constructor.
     * 
     * @param node the original node used for deletion
    */
   void clear(KDTreeNode* node);

    /**
     * Helper for KDTree FindNearestNeighbor.
     * 
     * @param query The point we wish to find the closest neighbor to in the
     *  tree.
     * @param node Current node passed into
     * @param dim dimension
    */ 
  ...
  typename KDTree<Dim>::KDTreeNode * quickSelect(vector<Point<Dim>>& points, int left_idx, int right_idx, int med, int d);
  typename KDTree<Dim>::KDTreeNode * buildTree(vector<Point<Dim>>& points, int left_idx, int right_idx, int d);
};
